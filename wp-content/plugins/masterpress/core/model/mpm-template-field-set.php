...
class MPM_TemplateFieldSet extends MPM_FieldSet {
  ...
  public function infer_position() {
    ...
    */
    ...
    global $wpdb;
    $max_pos = $wpdb->get_var("SELECT MAX(`position`) FROM `".MPM::table("field-set")."` WHERE `type` = 't'");
  ...
  public static function find_by_name($name) {
    return self::find( array( "where" => "`name` = '{$name}' AND `type` = 't'" ) ); 
  }

  public static function find_by_template($template, $orderby = "position ASC") {
    return self::find( array( "where" => MPM::visibility_rlike("templates", $template)." AND `type` = 't'", "orderby" => $orderby ) ); 
  }
  ...
  public function validate_name() {
    ...
    $result = call_user_func_array( array($class, "find_by_name_and_template"), array($this->name, $this->visibility["templates"]));

    if ($result && count($result)) {

      if ( ( $op == "insert" ) || ( $result[0]->id != $this->id ) ) {
        $this->err(sprintf(__('Sorry, a template field set named <em>%s</em> already exists for the template <span class="tt">%s</span>. Please enter a unique name', MASTERPRESS_DOMAIN), $this->name, $this->visibility["templates"]), "name");
      }
    }

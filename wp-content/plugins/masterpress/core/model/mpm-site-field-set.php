...
class MPM_SiteFieldSet extends MPM_FieldSet {
  ...
  public function infer_position() {
    ...
    */
    ...
    global $wpdb;
    $max_pos = $wpdb->get_var("SELECT MAX(`position`) FROM `".MPM::table("field-sets")."` WHERE `type` = 'w'");
  ...
  public static function find_by_name($name) {
    return self::find( array( "where" => "`name` = '{$name}' AND `type` = 'w'" ) ); 
  }
  ...
  public function validate_name($op) {
    if ($this->name != "") {
      ...
      $result = self::find( array("where" => "`type` = 'w' AND `name` = '{$this->name}'". $extra_check ) );

      if (isset($result) && $result) {
        $this->err(sprintf(__("Sorry, a site field set named <em>%s</em> already exists", MASTERPRESS_DOMAIN), $this->name), "name");
      }

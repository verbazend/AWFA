...
class MPV_TaxonomyFieldSets extends MPV {

  public static function __s() {
    return __("Taxonomy Field Set", MASTERPRESS_DOMAIN);
  }

  public static function __p() {
    return __("Taxonomy Field Sets", MASTERPRESS_DOMAIN);
  }
  ...
  public function confirm_delete_field($field) {
    ...
    MPV_FieldSets::confirm_delete_field_panel($field, __("Taxonomy Field", MASTERPRESS_DOMAIN), MPV::action_link("taxonomies", "manage-field-sets", __("Cancel", MASTERPRESS_DOMAIN), "parent=".$parent, array( "class" => "button button-small primary" )) );
  }
  ...
  public function confirm_delete($field_set) {
    ...
    $info = MasterPress::$view;
    $parent = MasterPress::$parent;
    MPV_FieldSets::confirm_delete_panel($field_set, __("Taxonomy Field Set", MASTERPRESS_DOMAIN), MPV::action_link("taxonomies", "manage-field-sets", __("Cancel", MASTERPRESS_DOMAIN), "parent=".$parent, array( "class" => "button button-small primary" )) );
  }
  ...
  public function grid() {
    ...
    <?php $field_sets = MPM_TaxonomyFieldSet::find_by_taxonomy( $tax, "name" ); ?>
    ...
    $has_actions = MasterPress::current_user_can("edit_taxonomy_field_sets,delete_taxonomy_field_sets,edit_taxonomy_fields,delete_taxonomy_fields");

    $can_edit = MasterPress::current_user_can("edit_taxonomy_field_sets");
    $can_delete = MasterPress::current_user_can("delete_taxonomy_field_sets");
    $can_create = MasterPress::current_user_can("create_taxonomy_field_sets");

    $can_edit_fields = MasterPress::current_user_can("edit_taxonomy_fields");
    $can_delete_fields = MasterPress::current_user_can("delete_taxonomy_fields");
    $can_create_fields = MasterPress::current_user_can("create_taxonomy_fields");
    ...
    $meta = $can_edit ? "{ href: '".MasterPress::admin_url("taxonomies", "edit-field-set", array("id" => $field_set->id, "parent" => $taxonomy_id) )."' }" : "";
    ...
    <tr <?php echo $title ?> class="<?php echo $disabled ?> <?php echo $editable_class.$deleting_class ?> <?php echo MPV::updated_class("edit-field-set,create-field-set", $field_set->id) ?> <?php echo $meta ?>">
      ...
      <?php else: ?>
        <?php if ($can_edit) : ?>
          <?php echo MPV::action_button("taxonomies", "edit-field-set", self::__edit(), array("id" => $field_set->id, "parent" => $taxonomy_id), array("class" => "button button-edit") ); ?>
        <?php endif; ?>

        <?php if ($can_delete) : ?>
          <?php echo MPV::action_button("taxonomies", "delete-field-set", self::__delete(), array("id" => $field_set->id, "parent" => $taxonomy_id), array("class" => "button button-delete") ); ?>
        <?php endif; ?>
      <?php endif; ?>
      </div>
      </th>
    ...
    $meta = $can_edit ? "{ href: '".MasterPress::admin_url("taxonomies", "edit-field", array("id" => $field->id, "gparent" => $_GET["parent"], "parent" => $field_set->id) )."' }" : "";
    ...
    <tr <?php echo $title ?> class="sub <?php echo $editable_class.$deleting_class ?> <?php echo $disabled ?> <?php echo $first ?> <?php echo $count % 2 == 0 ? "even" : "" ?> <?php echo MPV::updated_class("edit-field,create-field", $field->id) ?> <?php echo $meta ?>">
      ...
      <td class="label"><?php echo $field->display_label() ?></td>
      ...
      <div>
        <?php if ($can_edit_fields) : ?>
          <?php echo MPV::action_button("taxonomies", "edit-field", self::__edit(), array("id" => $field->id, "gparent" => $_GET["parent"], "parent" => $field_set->id), array("class" => "button button-edit") ); ?>
        <?php endif; ?>

        <?php if ($can_delete_fields) : ?>
          <?php echo MPV::action_button("taxonomies", "delete-field", self::__delete(), array("id" => $field->id, "gparent" => $_GET["parent"], "parent" => $taxonomy_id), array("class" => "button button-delete") ); ?>
        <?php endif; ?>
      </div>
      <?php endif; ?>
      </td>
    ...
    <tr class="summary <?php echo $can_create_fields ? "editable" : "" ?>">
      ...
      <?php echo MPV::action_button("taxonomies", "create-field", self::__create(MPV_Fields::__s()), array("gparent" => $_GET["parent"], "parent" => $field_set->id), array("class" => "button create-field")  ); ?>
      </td>
    ...
    <?php echo MPV::action_button("taxonomies", "create-field-set", MPV::__create( MPV_TaxonomyFieldSets::__s() ), array( "parent" => $taxonomy_id ) ); ?>
    </div>
    <?php endif; ?>
    ...
    <?php if ($can_create) : ?>
    <a href="<?php echo MasterPress::admin_url("taxonomies", "create-field-set", array("parent" => MasterPress::$parent ) ) ?>" class="no-items-create">
	<i class="plus-circle"></i>
    <span><?php printf( __( "The '<em>%s</em>' taxonomy currently has <em>no field sets</em>. Click here to create one.", MASTERPRESS_DOMAIN ), $tax->display_label() ) ?></span>
    </a>
    <?php endif; ?>
  ...
  public function form() {
    MPV::incl("field-sets");
    MasterPress::$model->taxonomy = true;

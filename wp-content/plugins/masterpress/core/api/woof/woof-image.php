...
class WOOF_Image extends WOOF_File {
  ...
  */
  ...
  public function srcset($args = array()) {
    
    $r = wp_parse_args($args, array("2x" => false));
    ...
    foreach ($r as $key => $value) {
      ...
      if (preg_match( "/2x-(\d+)w-(\d+)h/", $key, $matches)) {
      ...
      if (preg_match( "/2x-(\d+)w/", $key, $matches)) {

        $is_2x = true;
        $w = $matches[1];
        
      } else if (preg_match( "/2x-(\d+)h/", $key, $matches)) {
        
        $is_2x = WOOF::is_true_arg($r, "2x");
        $h = $matches[1];
      ...
      } else if (preg_match( "/(\d+)w-(\d+)h/", $key, $matches ) ) {
        
        $w = $matches[1];
        $h = $matches[2];
      
      } else if (preg_match( "/(\d+)w/", $key, $matches ) ) {
        
        $h = $matches[1];
        
      } else if (preg_match( "/(\d+)h/", $key, $matches ) ) {
      ...
      if ($is_2x) {
        ...
        // output the 2x
        // $inner_html .= WOOF_HTML::tag("div", array("data-src" => $url, "data-media" => implode(" and ", $media).$and."(min-device-pixel-ratio: 2.0)"), "" );
        // $inner_html .= WOOF_HTML::tag("div", array("data-src" => $url, "data-media" => implode(" and ", $media).$and."(-webkit-min-device-pixel-ratio: 1.5)"), "" );
    ...
    $r["data-height"] = $image_1x->height();
    $r["data-width"] = $image_1x->width();
    
    $r["srcset"] = implode(", ", $srcset);
    
    return $image_1x->tag( $r );
  ...
  function empty_mp_thumb($args = "w=60") {

...
class WOOF_PostType extends WOOF_Collection {
  ...
  function flatten_posts( $args = array() ) {
    ...
    if (WOOF::is_true_arg($r, "nocache") || is_null($ret = $this->property_cache_args("flatten_posts", $args))) {
      ...
      $this->property_cache_args("flatten_posts", $args, $ret);
  ...
  function cache_data($expires = "1y") {
    ...
    if ($data = $this->property_cache("data")) {
  ...
  function data($slug, $name, $value = null) {
    ...
    if (!$data = $this->property_cache("data")) {
      $pc = false; // this was not property cached
    ...
    // property-cache the data
    $this->property_cache("data", $data);
